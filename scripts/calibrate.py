...
By default, scores/recommendations are trusted as archived — but those were
stamped by whatever model version was live that day, so a stale label can
linger in old archives after a scoring change ships. --rescore instead
re-scores the whole archive walk-forward with the CURRENT production model
(reusing backtest_value.py's score_and_recommend + strike_rates.iter_history,
the same no-leakage rolling StrikeTable), so its numbers reconcile exactly
with `backtest_value.py --bootstrap`'s "new" arm:
    python3 scripts/calibrate.py --rescore
...
import backtest_value as bv  # --rescore: re-score with the current model
import strike_rates as sr    # --rescore: no-leakage walk-forward history
...
def print_report(bands, recs, n_dates, n_races_matched, n_races_total, pf_frac):
...
def run_rescore(out_dir: str, min_date: str, burn_in: int, pf_frac: float):
    """Re-score the whole archive walk-forward with the CURRENT production
    model (backtest_value.score_and_recommend, use_strike=True — the shipped
    arm), rolling the StrikeTable forward one day at a time with no leakage.
    Reconciles exactly with `backtest_value.py --bootstrap`'s "new" arm,
    because it's the identical code path over the identical burn-in window.
    """
    hist_dir = os.path.join(out_dir, 'history')
    days = sr.iter_history(hist_dir)
    if min_date:
        days = [(d, p) for d, p in days if d >= min_date]
    if not days:
        print('No joined history found (need races_*.json + results_full_*.json).',
              file=sys.stderr)
        return

    bands = defaultdict(new_band)
    recs = defaultdict(new_rec)
    ...
    for i, (date_str, praces) in enumerate(days):
        ...
        # live or burn-in alike — mirrors backtest_value.py's walk_forward)
        for prace in praces:
            table.add_race(prace)

    print_report(bands, recs, n_dates, n_races_matched, n_races_total, pf_frac)
...
def main():
    ...
    ap.add_argument('--rescore', action='store_true',
                    help='Re-score the archive walk-forward with the current '
                         'model instead of trusting each day\'s stored (possibly '
                         'stale) score/recommendation')
    ap.add_argument('--burn-in', type=int, default=15,
                    help='Warm-up days excluded from ROI (--rescore only, default 15)')
    args = ap.parse_args()
    ...
    if args.rescore:
        run_rescore(out_dir, args.min_date, args.burn_in, pf_frac)
        return
